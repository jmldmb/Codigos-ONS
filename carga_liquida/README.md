...
| Solar | perfil médio observado por mês (determinístico; expoente de forma 1,0 — o 1,5 do legado dava pico +10 % e dia 1 h mais curto); fator diário opcional (`solar_fator_diario`, std de D 0,10 cent. / 0,08 dist.); zeros noturnos ficam no treino e dias com artefato noturno do ONS são descartados | COFF FV / geração usina não-MMGD; MMGD |
...
5. Solar (bateria `validar --perfis`): o expoente 1,5 da centralizada distorcia a forma (MAE por hora 0,09, pico 2,95 vs
   ...
   (o pico exagerado inflava a sobra do meio-dia). Ficam fora: nível diário (std 0,10 / 0,08, `solar_fator_diario`
   desligado), resíduo intradiário de nuvens (std 0,14 / 0,07) e a correlação com a eólica (+0,29; 0,5–0,6 em fev–abr). Efeito
