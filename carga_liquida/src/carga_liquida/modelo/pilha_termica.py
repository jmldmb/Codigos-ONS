"""Pilha térmica (merit order) mensal montada dos dados abertos do ONS.
    ...
    flexibilidade   : capacidade flexível = capacidade − inflexibilidade média (val_verifinflexibilidade) na janela;
                      a parte inflexível já está no inflexterm. Sem isso a nuclear (~2 GW, CVU 20-31) ficaria na base
                      da pilha e distorceria o preço — o legado compensava somando 3.500 MW ao despacho
...
def _capacidade_mensal() -> pd.DataFrame:
    """Por (ano, mes, cod): pmax = máximo da geração verificada; inflex = média da inflexibilidade verificada."""
    partes = []
    for df in ons.iterar_termica_despacho(["val_verifgeracao", "val_verifinflexibilidade"], colunas_extra=["cod_usinaplanejamento"]):
        ...
        g = df.groupby([df["din_instante"].dt.year.rename("ano"), df["din_instante"].dt.month.rename("mes"),
            ...
            pmax=("val_verifgeracao", "max"), inflex=("val_verifinflexibilidade", "mean"))
        partes.append(g)
    return pd.concat(partes).groupby(level=[0, 1, 2]).agg(pmax=("pmax", "max"), inflex=("inflex", "mean")).reset_index()
...
def montar_todas(rebuild: bool = False) -> dict[tuple[int, int], pd.DataFrame]:
    ...
    for per, g in cvu.groupby(cvu["dat_iniciosemana"].dt.to_period("M")):
        ...
        jan["potencia_usina"] = (jan["pmax"] - jan["inflex"]).clip(lower=0)
        p = ultima.merge(jan[["potencia_usina"]], left_on="cod_usinaplanejamento", right_index=True, how="inner")
        p = p[(p["val_cvu"] > 0) & (p["potencia_usina"] >= cfg["pilha_min_flex_mw"])]
        p = p.sort_values(["val_cvu", "potencia_usina"])
        if p.empty:
            continue
